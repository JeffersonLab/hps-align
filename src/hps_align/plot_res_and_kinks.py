...
def plot_res_and_kinks(
    ...
    if (doResiduals):
        ...
        for vol in ["top", "bottom"]:
            ...
            for ipl in range(len(ures_vs_u_plot_list)):
                if ipl == 0 or ipl == 1:
                    profile_plotter.plot_profileY(
                        ures_vs_u_plot_list[ipl], xtitle=ures_vs_u_title_list[ipl], rangeX=[-10, 10], rangeY=[-0.2, 0.2])
                    profile_plotter.plot_profileY(
                        ures_vs_v_plot_list[ipl], xtitle=ures_vs_v_title_list[ipl], rangeX=[-20, 20], rangeY=[-0.15, 0.15])
                else:
                    profile_plotter.plot_profileY(
                        ures_vs_u_plot_list[ipl], xtitle=ures_vs_u_title_list[ipl], rangeY=[-0.2, 0.2])
                    profile_plotter.plot_profileY(
                        ures_vs_v_plot_list[ipl], xtitle=ures_vs_v_title_list[ipl], rangeY=[-0.15, 0.15])
    ...
    trk_profile_plotter.plot_profileY(
        "p_vs_tanLambda_top", xtitle="tan(#lambda)", ytitle="p [GeV]", rangeY=[0., 5.], fit="[0] + [1]*x")
    trk_profile_plotter.plot_profileY(
        "p_vs_tanLambda_bottom", xtitle="tan(#lambda)", ytitle="p [GeV]", rangeY=[0., 5.], fit="[0] + [1]*x")

    trk_profile_plotter.plot_profileY(
        "d0_vs_tanLambda_top", xtitle="tan(#lambda) top", ytitle="d0 [mm]", rangeX=[0, 0.1], rangeY=[-1., 1.])
    trk_profile_plotter.plot_profileY(
        "d0_vs_tanLambda_bottom", xtitle="tan(#lambda) bot", ytitle="d0 [mm]", rangeX=[-0.1, 0], rangeY=[-1., 1.])

    trk_profile_plotter.plot_profileY(
        "d0_vs_phi_top", xtitle="#phi top", ytitle="d0 [mm]", rangeX=[0, 0.1], rangeY=[-1., 1.])
    trk_profile_plotter.plot_profileY(
        "d0_vs_phi_bottom", xtitle="#phi bot", ytitle="d0 [mm]", rangeX=[-0.1, 0], rangeY=[-1., 1.])

    trk_profile_plotter.plot_profileY(
        "p_vs_phi_top", xtitle="#phi", ytitle="p [GeV]", rangeY=[0., 5.], fit="[0] + [1]*x")
    trk_profile_plotter.plot_profileY(
        "p_vs_phi_bottom", xtitle="#phi", ytitle="p [GeV]", rangeY=[0., 5.], fit="[0] + [1]*x")

    if (doEoPPlots):
        eop_profile_plotter = cfg.make_plotter(ProfilePlots, indir="EoP/")
        eop_profile_plotter.plot_profileY(
            "EoP_vs_trackP_top_fid", xtitle="Top track P [GeV]", rangeY=[0.7, 1.3], fit="[0]")

        eop_profile_plotter.plot_profileY("EoP_vs_trackP_ele_top_fid", xtitle="Top ele track P [GeV]", rangeY=[
        ...
        eop_profile_plotter.plot_profileY("EoP_vs_trackP_ele_bottom_fid", xtitle="Bot ele track P [GeV]", rangeY=[
                                          0.7, 1.3], fit="[0]", fitrange=[0.7, 1.2])

        eop_profile_plotter.plot_profileY(
            "EoP_vs_trackP_pos_bottom_fid", xtitle="Bot pos track P [GeV]", rangeY=[0.7, 1.3], fit="[0]")

        eop_profile_plotter.plot_profileY("EoP_vs_tanLambda_fid", xtitle="track tan(#lambda) [GeV]", rangeX=[
                                          -0.07, 0.07], rangeY=[0.7, 1.3], fit="[0]*x*x*x + [1]*x*x + [2]*x + [3]")

        eop_profile_plotter.plot_profileY("EoP_vs_phi_fid", xtitle="track #phi [GeV]", rangeY=[
                                          0.5, 1.3], fit="[0]*x*x*x + [1]*x*x + [2]*x + [3]")

        eop_profile_plotter.plot_profileY("EoP_vs_phi_top_fid", xtitle="Top track #phi [GeV]", rangeY=[
                                          0.5, 1.3], fit="[0]*x*x*x + [1]*x*x + [2]*x + [3]")

        eop_profile_plotter.plot_profileY("EoP_vs_phi_bottom_fid", xtitle="Bottom track #phi [GeV]", rangeY=[
                                          0.5, 1.3], fit="[0]*x*x*x + [1]*x*x + [2]*x + [3]")
    ...
    if (doFEEs):
        ...
        fee_plots.plot_histos("trk_params/p_hole_top",
                              xtitle="e^{-} Hole side p [GeV]")
        fee_plots.plot_histos("trk_params/p_slot_bottom",
                              xtitle="e^{-} Slot side p [GeV]", ytitle="Tracks")
        fee_plots.plot_histos("trk_params/p_hole_bottom",
                              xtitle="e^{-} Hole side p [GeV]", ytitle="Tracks")
        fee_plots.plot_histos("trk_params/p5h_top",
                              xtitle="p [GeV]", ytitle="Tracks")
        fee_plots.plot_histos("trk_params/p6h_top",
                              xtitle="p [GeV]", ytitle="Tracks")
        fee_plots.plot_histos("trk_params/p7h_top",
                              xtitle="p [GeV]", ytitle="Tracks")
        # fee_plots.plot_histos("trk_params/p6h_top")
